...
<head>
  ...
  <meta name="co-author" content="Le Li Kruczek">
    ...
    <div class="rvt-layout__content [ rvt-p-top-xxl rvt-p-lr-md rvt-p-lr-xxl-md-up ]">
        <div class="rvt-prose">
            ...
            *************************************************************** -->
            ...
            <h1 class="rvt-m-top-xs">View Events</h1>
        </div>
                        ...
                        *************************************************** -->
                        ...
                        <div class="rvt-flex-md-up rvt-items-center rvt-p-bottom-xs rvt-p-lr-xxs rvt-border-bottom">
                            ...
                            *********************************************** -->
                            ...
                            <h2 class="rvt-grow-1 rvt-ts-18 rvt-text-regular">Current Events</h2>

                            <!-- **********************************************
                                Right-aligned action link
                            *********************************************** -->

                            <a href="create_event.php" class="rvt-flex rvt-items-center rvt-link-plain rvt-m-top-sm rvt-m-top-none-md-up">
                                ...
                                <span class="rvt-ts-14 rvt-m-left-xs">Create New Event</span>
                            </a>
                        </div>
                        ...
                        <?php
							...
							$query = "
							    ...
							    user2.user_id, 
							    event_name, 
        						    TIME(CONVERT_TZ(FROM_UNIXTIME(events.unix_start_time), @@global.time_zone, user1.time_zone)) AS event_start_time,
        						    TIME(CONVERT_TZ(FROM_UNIXTIME(events.unix_end_time), @@global.time_zone, user1.time_zone)) AS event_end_time,
        						    DATE(CONVERT_TZ(FROM_UNIXTIME(events.unix_start_time), @@global.time_zone, user1.time_zone)) AS event_date,
							    event_location, 
							    event_description, 
							    organizer 
							    FROM `participants`, `events` 
							    JOIN users user1 ON user1.user_id=".$_SESSION['session_user_id']."
							    JOIN users user2 ON user2.user_email=events.organizer
						            WHERE participants.event_id = events.event_id 
							    AND participants.user_id=".$_SESSION['session_user_id'].";";
							$result = $conn->query($query);

							// Process the result and display events
    							...
    							<div class="rvt-timeline">
      								<div class="rvt-timeline__item">
        								...
        								<div class="rvt-timeline__content">
          									<h1 class="rvt-timeline__heading">' . $row['event_name'] . '</h1>
          									<span class="rvt-timeline__date">' . $row['event_date'] . '</span>
          									<p>Time: ' . $row['event_start_time'] . ' - ' . $row['event_end_time'] . '</p>
          									<p>Organized by: ' . $row['user_name'] . '</p>
          									<p>Location: ' . $row['event_location'] . '</p>
          									<p class="rvt-prose"> Event Description: ' . nl2br($row['event_description']) . '</p>
								 ';
							if ($row['user_id']==$_SESSION['session_user_id']){
								echo '
                     						<div class="rvt-flex-md-up rvt-items-center rvt-p-bottom-xs rvt-p-lr-xxs rvt-border-bottom">
                            					<h2 class="rvt-grow-1 rvt-ts-14 rvt-text-regular"><button class="rvt-button" onclick="redirectToEditPage('.$row['event_id'].')" >Modify This Event</button></h2>
                            					<h2 class="rvt-grow-1 rvt-ts-14 rvt-text-regular"><button class="rvt-button rvt-button--danger" type="button" onclick="deleteEvent('.$row['event_id'].')" >Delete This Event</button></h2>
                     						</div>
								 	 ';
							}
							echo '
							...
							<script>
    							function redirectToEditPage(event_id) {
    							...
    							function deleteEvent(event_id) {
									var result = confirm("Are you sure you want to delete this event?");
      								...
      								if (result) {
        								...
        								alert("Event deleted successfully!"); // Replace this line with your actual delete logic
      								} else {
